...
# independent Bayesian using Stan
...
params <- c("p1", "p2", "thetaRR", "thetaDiff")
...
out <- stan(data = dat_input,
            pars = params,
            file = here::here("stan", "independent-p.stan"),
            chains = 1,
            iter = n_iter,
            warmup = n_burnin,
            thin = n_thin,
...
save(stan_output, file = here::here("data", "stan_output.RData"))
